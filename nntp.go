...
// Copied from https://github.com/chrisfarms/nntp/tree/master
...
func (r *bodyReader) Read(p []byte) (n int, err error) {
	...
	if r.buf.Len() == 0 {
		...
		// stop on ".\r\n" or ".\n"
		if bytes.Equal(b, dotcrlf) || bytes.Equal(b, dotnl) {
			r.eof = true
			return 0, io.EOF
		}
		// unescape leading ..
...
func newConn(c net.Conn) (res *Conn, err error) {
	// Set a temporary deadline for the welcome banner. If the server
	// accepts the TCP/TLS connection but never sends the banner, we'd
	...
	// Clear the banner deadline so callers start with a clean slate.
...
func DialTLS(network, addr string, config *tls.Config) (*Conn, error) {
	// Use a dialer with a sane timeout to avoid hanging on connect/handshake
	d := &net.Dialer{Timeout: 10 * time.Second}
	c, err := tls.DialWithDialer(d, network, addr, config)
	if err != nil {
		return nil, err
	}
	// return nntp Conn
...
// Deadline control helpers. If the underlying conn supports deadlines (net.Conn),
// these methods will apply them. Otherwise they are no-ops.
...
// The article is in NNTP wire format with CRLF line endings.
